...
// - This circuit is the v2 TruthFinder circuit semantics, not a bit-match floating
//   implementation of TruthFinder.py.
// - It uses fixed Q16 arithmetic, frozen _tau_circuit (ApproxTauQ16), and frozen
//   _sigmoid_circuit (ApproxSigmoidQ16Signed).
// - It constructs one-hot support from top1_choice_flat inside the circuit.
// - It initializes s_state[0] with init_last_s.
// - It uses dep_avg for tau damping.
// - It uses imp_flat/conf_flat as split signed relation weights.
// - It uses trust-prior-smoothed model trust update.
// - It runs fixed ITER_N=15 rounds.
// - Natural-language normalization, candidate clustering, signed relation construction,
//   and dep_avg construction are outside the circuit.
...
template Q16DivNonNeg() {
    // If den > 0: out = floor(num * Q16 / den)
    // If den = 0: out = fallback
    var Q16 = 65536;
    ...
    scaledNum === q * denEff + r;

    component rlt = LessThan(64);
    rlt.in[0] <== r;
    rlt.in[1] <== denEff;
    rlt.out === 1;
...
template TruthFinderRound() {
    ...
    signal input trust_prior_default;
    signal input trust_prior_strength;
    ...
    // imp_flat/conf_flat are produced outside the circuit by splitting signed
    // relation: rel > 0 -> imp, rel < 0 -> conf = -rel. The circuit only proves
    // that these relation weights are correctly used in fixed-round TruthFinder computation.
    ...
    component numSum[M];
    component denSum[M];
    component priorMul[M];
    signal numeratorWithPrior[M];
    signal denominatorWithPrior[M];
    component div[M];
    component tClamp[M];
    ...
    // update t as trust-prior-smoothed weighted average of s over support
    for (var w4 = 0; w4 < M; w4++) {
        ...
        priorMul[w4] = Q16Mul();
        priorMul[w4].a <== trust_prior_strength;
        priorMul[w4].b <== trust_prior_default;

        numeratorWithPrior[w4] <== numSum[w4].out + priorMul[w4].out;
        denominatorWithPrior[w4] <== denSum[w4].out + trust_prior_strength;

        div[w4] = Q16DivNonNeg();
        div[w4].num <== numeratorWithPrior[w4];
        div[w4].den <== denominatorWithPrior[w4];
        div[w4].fallback <== trust_prior_default;
...
template TruthFinderMain() {
    ...
    signal input fact_count_by_object[K_MAX];
    signal input is_effective_by_object[K_MAX];
    signal input top1_choice_flat[K_MAX * M];
    ...
    component kBits = Num2Bits(8);
    component kBound = LessThan(8);
    component oLtK[K_MAX];
    component fcBits[K_MAX];
    component fcBound[K_MAX];
    component fcZero[K_MAX];
    component top1Zero[K_MAX][M];
    component top1PlusBits[K_MAX][M];
    component top1LeCount[K_MAX][M];
    component top1Bound[K_MAX][M];
    component depBits[M];
    signal top1_plus1[K_MAX][M];
    component depBound[M];
    component impBits[K_MAX][N_MAX][N_MAX];
    component confBits[K_MAX][N_MAX][N_MAX];
    component impBound[K_MAX][N_MAX][N_MAX];
    component confBound[K_MAX][N_MAX][N_MAX];
    component relGValid[K_MAX][N_MAX][N_MAX];
    component relFValid[K_MAX][N_MAX][N_MAX];
    signal relFactMask[K_MAX][N_MAX][N_MAX];
    signal relValidMask[K_MAX][N_MAX][N_MAX];
    component geM[M];
    component winnerLtCount[K_MAX];
    component fValid[K_MAX][N_MAX];
    component geF[K_MAX][N_MAX];
    component supportFactValid[K_MAX][N_MAX];
    component eqTop1[K_MAX][N_MAX][M];
    signal supportMask[K_MAX][N_MAX];
    signal support_flat_internal[K_MAX * N_MAX * M];
    component initFactValid[K_MAX][N_MAX];
    signal initMask[K_MAX][N_MAX];

    signal mustHold[K_MAX][N_MAX];

    // ---------------- Frozen parameter constraints ----------------
    t0 === 49152;
    beta === 22938;
    gamma === 19661;
    alpha_imp === 13107;
    ...
    for (var wDep = 0; wDep < M; wDep++) {
        depBits[wDep] = Num2Bits(17);
        depBits[wDep].in <== dep_avg[wDep];

        depBound[wDep] = LessThan(32);
        depBound[wDep].in[0] <== dep_avg[wDep];
        depBound[wDep].in[1] <== Q16 + 1;
        depBound[wDep].out === 1;
    }
    ...
    for (var o = 0; o < K_MAX; o++) {
        ...
        fcBits[o] = Num2Bits(8);
        fcBits[o].in <== fact_count_by_object[o];
        ...
        for (var w = 0; w < M; w++) {
            var top1Idx = o * M + w;
            top1_plus1[o][w] <== top1_choice_flat[top1Idx] + 1;

            top1Zero[o][w] = IsZero();
            top1Zero[o][w].in <== top1_plus1[o][w];

            top1PlusBits[o][w] = Num2Bits(8);
            top1PlusBits[o][w].in <== top1_plus1[o][w];

            top1LeCount[o][w] = LessThan(8);
            top1LeCount[o][w].in[0] <== top1_plus1[o][w];
            top1LeCount[o][w].in[1] <== fact_count_by_object[o] + 1;

            top1Bound[o][w] = LessThan(8);
            top1Bound[o][w].in[0] <== top1_plus1[o][w];
            top1Bound[o][w].in[1] <== N_MAX + 1;
            top1Bound[o][w].out === 1;

            // effective object => top1 != -1 and top1+1 <= fact_count
            is_effective_by_object[o] * top1Zero[o][w].out === 0;
            is_effective_by_object[o] * (1 - top1LeCount[o][w].out) === 0;

            // padding object => top1 == -1
            (1 - is_effective_by_object[o]) * top1_plus1[o][w] === 0;
        }
    }

    // Build one-hot support from top1_choice_flat inside the circuit.
    for (var oSupport = 0; oSupport < K_MAX; oSupport++) {
        for (var fSupport = 0; fSupport < N_MAX; fSupport++) {
            supportFactValid[oSupport][fSupport] = LessThan(8);
            supportFactValid[oSupport][fSupport].in[0] <== fSupport;
            supportFactValid[oSupport][fSupport].in[1] <== fact_count_by_object[oSupport];

            supportMask[oSupport][fSupport] <==
                is_effective_by_object[oSupport] * supportFactValid[oSupport][fSupport].out;

            for (var wSupport = 0; wSupport < M; wSupport++) {
                var idxSupport = ((oSupport * N_MAX) + fSupport) * M + wSupport;
                var top1SupportIdx = oSupport * M + wSupport;

                eqTop1[oSupport][fSupport][wSupport] = IsZero();
                eqTop1[oSupport][fSupport][wSupport].in <==
                    top1_choice_flat[top1SupportIdx] - fSupport;

                support_flat_internal[idxSupport] <==
                    Q16 * supportMask[oSupport][fSupport] * eqTop1[oSupport][fSupport][wSupport].out;
            }
        }
    }
    ...
    for (var o2 = 0; o2 < K_MAX; o2++) {
        for (var f = 0; f < N_MAX; f++) {
            initFactValid[o2][f] = LessThan(8);
            initFactValid[o2][f].in[0] <== f;
            initFactValid[o2][f].in[1] <== fact_count_by_object[o2];

            initMask[o2][f] <== is_effective_by_object[o2] * initFactValid[o2][f].out;
            s_state[0][o2][f] <== init_last_s * initMask[o2][f];
        }
    }

    // fixed 15 rounds (project-fixed ITER_N)
    ...
    for (var it = 0; it < ITER_N; it++) {
        ...
        rounds[it].trust_prior_default <== trust_prior_default;
        rounds[it].trust_prior_strength <== trust_prior_strength;
        ...
        for (var i = 0; i < K_MAX * N_MAX * M; i++) {
            rounds[it].support_flat[i] <== support_flat_internal[i];
        }
    ...
    for (var o6 = 0; o6 < K_MAX; o6++) {
        for (var g6 = 0; g6 < N_MAX; g6++) {
            for (var f6 = 0; f6 < N_MAX; f6++) {
                var relIdx = ((o6 * N_MAX) + g6) * N_MAX + f6;

                impBits[o6][g6][f6] = Num2Bits(17);
                impBits[o6][g6][f6].in <== imp_flat[relIdx];

                impBound[o6][g6][f6] = LessThan(32);
                impBound[o6][g6][f6].in[0] <== imp_flat[relIdx];
                impBound[o6][g6][f6].in[1] <== Q16 + 1;
                impBound[o6][g6][f6].out === 1;

                confBits[o6][g6][f6] = Num2Bits(17);
                confBits[o6][g6][f6].in <== conf_flat[relIdx];

                confBound[o6][g6][f6] = LessThan(32);
                confBound[o6][g6][f6].in[0] <== conf_flat[relIdx];
                confBound[o6][g6][f6].in[1] <== Q16 + 1;
                confBound[o6][g6][f6].out === 1;

                imp_flat[relIdx] * conf_flat[relIdx] === 0;

                relGValid[o6][g6][f6] = LessThan(8);
                relGValid[o6][g6][f6].in[0] <== g6;
                relGValid[o6][g6][f6].in[1] <== fact_count_by_object[o6];

                relFValid[o6][g6][f6] = LessThan(8);
                relFValid[o6][g6][f6].in[0] <== f6;
                relFValid[o6][g6][f6].in[1] <== fact_count_by_object[o6];

                relFactMask[o6][g6][f6] <==
                    relGValid[o6][g6][f6].out
                    * relFValid[o6][g6][f6].out;

                relValidMask[o6][g6][f6] <==
                    is_effective_by_object[o6]
                    * relFactMask[o6][g6][f6];

                imp_flat[relIdx] * (1 - relValidMask[o6][g6][f6]) === 0;
                conf_flat[relIdx] * (1 - relValidMask[o6][g6][f6]) === 0;
