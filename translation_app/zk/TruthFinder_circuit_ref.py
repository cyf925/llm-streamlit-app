...
def _require_range(x: int, lo: int, hi: int, field: str) -> None:
    if x < lo or x > hi:
        raise CircuitRefError(f"{field} out of range [{lo},{hi}]: {x}")
...
def q16_div_nonneg(num_q16: int, den_q16: int, fallback_q16: int) -> int:
    """Divide two non-negative Q16 values and return a Q16 quotient."""
    if den_q16 == 0:
        return int(fallback_q16)
    return (int(num_q16) * Q16) // int(den_q16)
...
def _validate_runtime_contract(ci: CircuitInput) -> int:
    ...
    if support_mode != "top1_in_circuit":
        raise CircuitRefError(
            "runtime.support_mode must be 'top1_in_circuit'; "
            f"this reference only supports top1 support constructed in circuit/reference, got {support_mode!r}"
        )
...
def _parse_params_q16(ci: CircuitInput) -> Dict[str, int]:
    out: Dict[str, int] = {}
    for k, expected in EXPECTED_PARAMS_Q16.items():
        ...
        if out[k] != expected:
            raise CircuitRefError(
                f"params_q16.{k} must equal fixed circuit value {expected}, got {out[k]}"
            )
    return out
...
def _parse_object_meta(ci: CircuitInput, K: int) -> Dict[str, List[int]]:
    ...
    for o in range(K_MAX_FIXED):
        ...
        for w in range(M_FIXED):
            ...
            top1 = top1_flat[idx]
            if expected_eff == 1:
                if top1 < 0 or top1 >= fc:
                    raise CircuitRefError(
                        f"top1_choice_flat[{idx}]={top1} invalid for effective object o={o}, "
                        f"model={w}, fact_count={fc}"
                    )
            else:
                if top1 != -1:
                    raise CircuitRefError(
                        f"top1_choice_flat[{idx}] for padding object o={o}, model={w} must be -1, got {top1}"
                    )
...
def build_support_from_top1(
    top1_flat: Sequence[int],
    fact_count_by_object: Sequence[int],
    is_effective_by_object: Sequence[int],
) -> List[List[List[int]]]:
    if len(top1_flat) != K_MAX_FIXED * M_FIXED:
        raise CircuitRefError(f"top1_flat length must be {K_MAX_FIXED * M_FIXED}, got {len(top1_flat)}")
    if len(fact_count_by_object) != K_MAX_FIXED:
        raise CircuitRefError(
            f"fact_count_by_object length must be {K_MAX_FIXED}, got {len(fact_count_by_object)}"
        )
    if len(is_effective_by_object) != K_MAX_FIXED:
        raise CircuitRefError(
            f"is_effective_by_object length must be {K_MAX_FIXED}, got {len(is_effective_by_object)}"
        )

    support = [[[0 for _ in range(M_FIXED)] for _ in range(N_MAX_FIXED)] for _ in range(K_MAX_FIXED)]
    for o in range(K_MAX_FIXED):
        if int(is_effective_by_object[o]) != 1:
            continue
        fc = int(fact_count_by_object[o])
        for w in range(M_FIXED):
            idx = o * M_FIXED + w
            f = int(top1_flat[idx])
            if f < 0 or f >= fc:
                raise CircuitRefError(
                    f"cannot build support: top1_choice_flat[{idx}]={f} invalid for object={o}, fact_count={fc}"
                )
            support[o][f][w] = Q16
    return support
...
def _parse_trust_prior(ci: CircuitInput, params: Mapping[str, int]) -> List[int]:
    raw = ci.circom_arrays.get("trust_prior")
    default = int(params["trust_prior_default"])
    if raw is None:
        return [default for _ in range(M_FIXED)]
    if not isinstance(raw, list) or len(raw) != M_FIXED:
        raise CircuitRefError(f"circom_arrays.trust_prior must be len={M_FIXED}")
    out = [_as_int(v, f"circom_arrays.trust_prior[{i}]") for i, v in enumerate(raw)]
    for i, val in enumerate(out):
        if val != default:
            raise CircuitRefError(
                "circom_arrays.trust_prior must match uniform trust_prior_default in v2 circuit semantics; "
                f"index {i} got {val}, expected {default}"
            )
    return out


def _parse_arrays(ci: CircuitInput, K: int, params: Mapping[str, int]) -> ParsedArrays:
    ...
    trust_prior = _parse_trust_prior(ci, params)

    for w, val in enumerate(dep_avg):
        _require_range(val, 0, Q16, f"circom_arrays.dep_avg[{w}]")
    for w, val in enumerate(trust_prior):
        _require_range(val, 0, Q16, f"circom_arrays.trust_prior[{w}]")
    for o in range(K_MAX_FIXED):
        for g in range(N_MAX_FIXED):
            for f in range(N_MAX_FIXED):
                _require_range(imp[o][g][f], 0, Q16, f"circom_arrays.imp_flat[o={o},g={g},f={f}]")
                _require_range(conf[o][g][f], 0, Q16, f"circom_arrays.conf_flat[o={o},g={g},f={f}]")
                if g == f:
                    if imp[o][g][f] != 0:
                        raise CircuitRefError(
                            f"circom_arrays.imp_flat diagonal must be 0 at object={o}, index={g}, got {imp[o][g][f]}"
                        )
                    if conf[o][g][f] != 0:
                        raise CircuitRefError(
                            f"circom_arrays.conf_flat diagonal must be 0 at object={o}, index={g}, got {conf[o][g][f]}"
                        )

    return ParsedArrays(K=K, imp=imp, conf=conf, dep_avg=dep_avg, trust_prior=trust_prior)


def _validate_relation_arrays_against_object_meta(
    arr: ParsedArrays,
    fact_count_by_object: Sequence[int],
    is_effective_by_object: Sequence[int],
    K: int,
    ...
    for o in range(K_MAX_FIXED):
        n_o = int(fact_count_by_object[o])
        eff = int(is_effective_by_object[o])
        for g in range(N_MAX_FIXED):
            for f in range(N_MAX_FIXED):
                imp = int(arr.imp[o][g][f])
                conf = int(arr.conf[o][g][f])

                if g == f:
                    if imp != 0 or conf != 0:
                        raise CircuitRefError(
                            f"relation diagonal must be zero at object={o}, g={g}, f={f}: imp={imp}, conf={conf}"
                        )

                is_padding_edge = (o >= K) or (eff == 0) or (g >= n_o) or (f >= n_o)
                if is_padding_edge and (imp != 0 or conf != 0):
                    raise CircuitRefError(
                        f"padding relation must be zero at object={o}, g={g}, f={f}: imp={imp}, conf={conf}"
                    )

                if imp != 0 and conf != 0:
                    raise CircuitRefError(
                        f"imp/conf must be exclusive at object={o}, g={g}, f={f}: imp={imp}, conf={conf}"
                    )
...
def run_truthfinder_circuit_ref(input_data: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    - fixed ITER_N=15 rounds
    - support constructed from top1_choice_flat inside reference/circuit semantics
    - imp/conf are split from signed relation outside circuit and used inside reference/circuit
    - Python TruthFinder may use early_stop, but this circuit reference uses fixed 15 rounds
    """
    ci = CircuitInput(
    ...
    init_last_s = params["init_last_s"]
    if "use_trust_prior" in ci.params_meta and ci.params_meta.get("use_trust_prior") is not True:
        raise CircuitRefError(
            f"params_meta.use_trust_prior must be True for v2 circuit semantics, got {ci.params_meta.get('use_trust_prior')!r}"
        )
    use_trust_prior = True

    fact_count_by_object = ometa["fact_count_by_object"]
    is_effective_by_object = ometa["is_effective_by_object"]
    _validate_relation_arrays_against_object_meta(
        arr,
        fact_count_by_object=fact_count_by_object,
        is_effective_by_object=is_effective_by_object,
        K=K,
    )
    support = build_support_from_top1(
        top1_flat=ometa["top1_choice_flat"],
        fact_count_by_object=fact_count_by_object,
        is_effective_by_object=is_effective_by_object,
    )
    ...
    for w in range(M_FIXED):
        t_state[0][w] = t0
    for o in range(K_MAX_FIXED):
        if is_effective_by_object[o] == 1:
            for f in range(fact_count_by_object[o]):
                s_state[0][o][f] = init_last_s
    ...
    for it in range(ITER_N_FIXED):
        ...
        for o in range(K_MAX_FIXED):
            for f in range(N_MAX_FIXED):
                base_sum = 0
                for w in range(M_FIXED):
                    base_sum += q16_mul(tau_w[w], support[o][f][w])
        ...
        for w in range(M_FIXED):
            ...
            for o in range(K_MAX_FIXED):
                for f in range(N_MAX_FIXED):
                    ...
                    den += sup
            mu = params["trust_prior_strength"]
            prior = arr.trust_prior[w]
            numerator = q16_mul(mu, prior) + num
            denominator = mu + den
            t_new = q16_div_nonneg(numerator, denominator, prior)
            t_out[w] = q16_clamp01(t_new)
    ...
    return {
        ...
        "meta": {
            ...
            "support_constructed_from_top1": True,
            "init_last_s_q16": params["init_last_s"],
            "use_trust_prior": use_trust_prior,
            "trust_prior": arr.trust_prior,
            "trust_prior_strength_q16": params["trust_prior_strength"],
            "circuit_semantics_version": "truthfinder-circuit-spec-v2-top1-support-trust-prior",
            "note": (
                "This output is from TruthFinder_circuit_ref.py, the circuit-semantic reference "
                "for the v2 truthfinder.circom design: Q16, frozen approximations, fixed 15 rounds, "
                "support constructed from top1_choice, init_last_s, and trust-prior-smoothed model trust update."
            ),
        },
    }


assert approx_sigmoid_q16_signed(393216) == 65372
assert approx_sigmoid_q16_signed(393217) == 65374
assert approx_sigmoid_q16_signed(-393216) == 162
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Reads truthfinder_circom_input.json, runs fixed Q16 ITER_N=15 rounds, "
